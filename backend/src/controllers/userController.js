...
export async function createUser(req, res, next) {
  ...
  try {
    ...
    const result = await pool.query(
      `INSERT INTO users (tenant_id, email, password_hash, full_name, role) VALUES ($1, $2, $3, $4, $5) RETURNING id, email, full_name, role, is_active, created_at`,
...
export async function updateUser(req, res, next) {
  ...
  try {
    ...
    const result = await pool.query(
      `UPDATE users SET ${updates.join(", ")}, updated_at = NOW() WHERE id = $${i} RETURNING id, email, full_name, role, is_active`,
      values
    );
...
export async function deleteUser(req, res, next) {
  ...
  try {
    ...
    if (req.user.role !== "tenant_admin" || req.user.tenantId !== userRes.rows[0].tenant_id) {
      return res.status(403).json({ success: false, message: "Unauthorized" });
    }
