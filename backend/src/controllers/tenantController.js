...
export async function listTenants(req, res, next) {
  ...
  try {
    const result = await pool.query(`
      SELECT t.id, t.name, t.subdomain, t.status, t.subscription_plan, t.created_at, COUNT(DISTINCT u.id) AS total_users, COUNT(DISTINCT p.id) AS total_projects
      FROM tenants t
...
export async function getTenant(req, res, next) {
  ...
  try {
    const result = await pool.query(
      `SELECT t.id, t.name, t.subdomain, t.status, t.subscription_plan, t.max_users, t.max_projects, COUNT(DISTINCT u.id) AS total_users, COUNT(DISTINCT p.id) AS total_projects, COUNT(DISTINCT tk.id) AS total_tasks
      FROM tenants t
      LEFT JOIN users u ON u.tenant_id = t.id
      LEFT JOIN projects p ON p.tenant_id = t.id
...
export async function updateTenant(req, res, next) {
  ...
  try {
    ...
    const result = await pool.query(
      `UPDATE tenants SET ${fields.join(", ")}, updated_at = NOW() WHERE id = $${index} RETURNING id, name, status, subscription_plan`,
      values
    );
